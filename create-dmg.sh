#!/usr/bin/env bash
# Build Casper.app and wrap it in a drag-to-Applications DMG.
set -euo pipefail
...
rm -rf "$STAGE"
mkdir -p "$STAGE"
cp -R "$APP" "$STAGE/Casper.app"
ln -s /Applications "$STAGE/Applications"

cat > "$STAGE/Install.txt" <<'EOF'
Casper — install

1. Drag Casper into Applications.
2. Open Casper from Applications (not this disk image).
3. First launch: if macOS blocks it, right-click the app → Open.
4. Grant Microphone and Accessibility (Casper.app, not Cursor).
...
echo
echo "DMG ready: $DMG"
echo "Give users that file. They drag Casper → Applications, then set keys in the app."
echo
